...
function _cleanupIfEntry(entry) {
   ...
   */
  var entry_if = _cleanupMethodDefinition(entry['if']);

  if (entry_if !== null) {
    var entry_then = entry.hasOwnProperty('then') ? _cleanupEntry(entry['then']) : null;
    var entry_else = entry.hasOwnProperty('else') ? _cleanupEntry(entry['else']) : null;
    if ((entry_then !== null) || (entry_else !== null)) {
      return {
        'label': entry.label,
        'if': entry_if,
        'then': entry_then,
...
function _cleanupLoopEntry(entry) {
   ...
   *                  function (reference)
   *
   * 'loop' := 'true' |                         -- Infinite Loop : Break will have to occur in the block
   ...
   * [OPTIONAL] 'on-success' := sequence_entry  -- Entry to Execute on Success (Loop Exited Without Errors)
   * [OPTIONAL] 'on-error'   := sequence_entry  -- Entry to Execute on Error (Does not call error on parent sequence, unless the sequence entry does)
   */
  ...
  if (control !== null) {
    ...
    if (block) {
      // Cleanup 'on-success'
      var onSuccess = entry.hasOwnProperty('on-success') ? _cleanupEntry(entry['on-success']) : null;
...
Sequence.prototype.end = function () {
  ...
  if (this.__errors !== null) { // YES: Call Error Handler (if it exists)
    ...
    return this.__onError !== null ? _wrappedCall(this.__context, this.__onError, this.__errors) : false;
  } else { // NO: Call the Success Handler for the Sequence (if it exists)
    return this.__onSuccess !== null ? _wrappedCall(this.__context, this.__onSuccess) : false;
  }
};
...
Sequence.prototype.__processEntry = function (entry) {
  ...
  if (entry.hasOwnProperty('error')) { // YES
    ...
    if(_isMethodCall(error)) { // YES
      /* Treat Method Call as async (i.e. the method will have to call .next()/.continue()/.error() as
       * it sees fit
       */
...
Sequence.prototype.__executeCheck = function (entry) {
  ...
  this.__loopBlock = null;
  return _.isBoolean(check) ? this.__executeBlock(entry['block']) : _callMethod(this.__context, check);
};
